    ...
    if not _missing_income_df.empty:
        with st.expander(f"📅 เดือนที่ยังขาดรายงานรายได้ ({len(_missing_income_df)} ร้าน)", expanded=True):
            st.caption(
                ...
                "(หรือมีแถวรายได้แล้วแต่ยอด ≤ 0 ซึ่งถือว่ายังไม่ปิดยอดเช่นกัน) — ตัวเลขในวงเล็บคือ "
                "จำนวนออเดอร์ค้างของเดือนนั้น เดือนล่าสุด (โดยเฉพาะ ~2 สัปดาห์หลังสุด) มักมีค้างเยอะเป็น"
                "ปกติเพราะรอรอบโอนเงินของแพลตฟอร์ม ไม่ใช่ลืมอัปโหลด — เดือนเก่าที่ยังมีเลขค้างอยู่ (ถึงจะ"
                "น้อย) นั่นแหละที่ควรไปโหลดรายงานรายได้มาอัปโหลดเพิ่มจริงๆ"
            )
            st.dataframe(
