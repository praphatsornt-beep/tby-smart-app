...
function doPost(e) {
  ...
  // "tu.3601.1" (จุดแทนขีด ทั้งคั่นหน้าเลขและคั่นจำนวน บางทีจุดคู่) — normalize เป็น
  // "ty2010-1"/"tf2581-1"/"tu2315-2"/"tu3601-1" ก่อน tokenize เสมอ เพราะรหัสสินค้าจริง
  // เป็น LETTERS+4DIGITS ล้วน ไม่มีขีด/จุดคั่นเอง (พอร์ตมาจาก
  ...
  // ลูกค้า (ไม่มีตัวอักษรนำหน้า) โดยไม่ตั้งใจ
  userMsg = userMsg.replace(/([a-z]{1,4})\s*[-.]{0,2}\s*(\d{4})\s*[-.=]{1,2}\s*(\d+(?:\.\d+)?)/g,
