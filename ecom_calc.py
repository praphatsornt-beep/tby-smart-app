...
def settled_order_sns(income_rows: list[dict]) -> set[str]:
    ...
    net_amount <= 0 (รวมถึง 0/None) ถือเป็น "ยังไม่ปิดยอด" เหมือนไม่มีแถว Income เลย ไม่ใช่
    "ปิดยอดแล้วที่ ฿0" — ตัดสินใจ 2026-09-21 หลังเจอจริงว่า TikTok มีแถว Income อยู่แล้วแต่
    net_amount=0 ทั้งที่ TikTok จ่ายเงินร้านจริง (บั๊กนำเข้าไฟล์เก่าก่อนแก้ 2026-09-05 อ่าน
    คอลัมน์ผิดชื่อ) การนับว่า "ปิดยอดแล้วที่ 0" ทำให้ออเดอร์เหล่านี้ขึ้นเป็นขาดทุนเต็มต้นทุน
    ผิดๆ ทั้งที่ยังไม่รู้ยอดจริง — แทบไม่เคยเจอเคสที่ net_amount=0 คือค่าที่ถูกต้องจริงๆ เลย
    สักครั้ง เลยเลือกถือว่า "รอโอน" เสมอทุกแพลตฟอร์ม ปลอดภัยกว่าเสี่ยงนับเป็นขาดทุนหลอก"""
