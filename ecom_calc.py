...
_ORDER_ITEM_RE = re.compile(
    ...
    # \s* (ไม่ใช่ \s+) หลัง (.*?) — พบเพิ่ม 2026-09-20 จากออเดอร์หลายสินค้าจริง #260920H92E7K5E:
    # ตัวเลือกสินค้า: ว่างเปล่าของ "สินค้าตัวที่ 1" ตามด้วย "ตัวเลือกสินค้า\s*:\s*" ที่ \s* (greedy)
    # กินช่องว่างเดียวระหว่างโคลอนกับคำว่า "จำนวน" ไปหมดแล้ว ถ้าบังคับ \s+ อีกตัวหลัง (.*?)
    # จะไม่มีช่องว่างเหลือให้ match ตรงนั้นเลย (.*?) เลยถูกบังคับ backtrack ขยายยาวข้าม
    # "จำนวน/ราคา" ของสินค้าตัวที่ 1 เอง ไปจนถึง "ตัวเลือกสินค้า:" ของสินค้าตัวถัดไป —ผลคือ
    # สินค้าตัวที่ 1 ได้ qty/price ของสินค้าตัวถัดไปไปแทน แล้วสินค้าตัวถัดไปหายไปจาก items
    # ทั้งแถว (ยืนยันจากข้อมูลจริงใน DB: order #260920H92E7K5E ควรมี 2 สินค้า แต่ parse ได้
    # แค่ 1 แถว โดยแถวนั้นมี price=242 ซึ่งเป็นราคาของสินค้าตัวที่ 2 ไม่ใช่ตัวที่ 1 จริง=378)
...
def settled_order_sns(income_rows: list[dict]) -> set[str]:
    """order_sn ทั้งหมดที่มีรายงานยอดโอน (Income) มายืนยันแล้ว "จริง" (net_amount != 0) —
    ใช้เช็คว่าออเดอร์นี้ "ปิดยอด" แล้วหรือยังรอ (ยังไม่มี Income = ยังไม่รู้ยอดจริง ไม่ควร
    เอามาคิดกำไร) แต่ละ dict ต้องมี "net_amount" (ไม่ใช่แค่ "order_sn" เฉยๆ เหมือนก่อนหน้านี้)

    net_amount == 0 เป๊ะ (รวมถึง None/ไม่มีแถวเลย) ถือเป็น "ยังไม่ปิดยอด" เหมือนไม่มีแถว
    Income เลย ไม่ใช่ "ปิดยอดแล้วที่ ฿0" — ตัดสินใจ 2026-09-21 หลังเจอจริงว่า TikTok มีแถว
    Income อยู่แล้วแต่ net_amount=0 ทั้งที่ TikTok จ่ายเงินร้านจริง (บั๊กนำเข้าไฟล์เก่าก่อนแก้
    2026-09-05 อ่านคอลัมน์ผิดชื่อ) การนับว่า "ปิดยอดแล้วที่ 0" ทำให้ออเดอร์เหล่านี้ขึ้นเป็น
    ขาดทุนเต็มต้นทุนผิดๆ ทั้งที่ยังไม่รู้ยอดจริง — แทบไม่เคยเจอเคสที่ net_amount=0 คือค่าที่
    ถูกต้องจริงๆ เลยสักครั้ง เลยเลือกถือว่า "รอโอน" เสมอทุกแพลตฟอร์ม

    **net_amount ติดลบไม่ใช่กรณีเดียวกัน — ต้องนับว่า "ปิดยอดแล้ว" ปกติ** (แก้ 2026-09-21
    วันเดียวกัน จากเดิมใช้ `> 0` ซึ่งกวาดค่าติดลบไปเป็น "รอโอน" ด้วยโดยไม่ตั้งใจ): พบจริง
    order #260723DMNYMNFR (ts_shop56) — ลูกค้าคืนสินค้าเต็มจำนวน (`return_status`="คำขอ
    ได้รับการยอมรับแล้ว"), Shopee โอนยอดสุทธิ net_amount=-102.0 มาจริง (ยืนยันด้วย
    transfer_date) นี่คือยอดขาดทุนที่ปิดจบแล้วจริงๆ (ค่าปรับจากการคืนสินค้า) ไม่ใช่ข้อมูล
    ที่ยังไม่มา — ถ้านับเป็น "รอโอน" จะซ่อนขาดทุนจริงไปจากทุกตาราง (order_anomaly_rows/
    aggregate_product_margin) อย่างผิดๆ พอดีกัน"""
    return {r["order_sn"] for r in income_rows if float(r.get("net_amount") or 0) != 0}
