...
def render():
    try:
        _t6_active = st.pills(" ", _T6_TABS, key="_t6_active_sub", default=_T6_TABS[0], label_visibility="collapsed") or _T6_TABS[0]
    ...
    if _t6_active == "📋 ของฝาก":
        ...
        with st.expander("📥 นำเข้า \"คอม\" จากรายงานบริษัท (.xls)"):
            st.caption(
                "อัปโหลดไฟล์ \"รายงานสินค้าคงเหลือปัจจุบัน/มูลค่า\" จากเมนู **รายงานคลังสินค้า** "
                "ในระบบบริษัท จะอัปเดตคอลัมน์ \"คอม\" ให้อัตโนมัติ (ไม่แตะ \"นับจริง\" ที่คีย์ไว้เดิม) — "
                "สินค้าที่มีอยู่แล้วในระบบแต่ไม่ปรากฏในไฟล์นี้จะถูกตั้ง \"คอม\" เป็น 0 ให้ด้วย ส่วนรหัส"
                "สินค้าในไฟล์ที่ยังไม่มีในระบบจะถูกเพิ่มเป็นสินค้าใหม่อัตโนมัติ (ราคาขาย/PV เป็น 0 ไว้"
                "ก่อน ต้องไปแก้ที่ ⚙️ จัดการข้อมูล → 🏷️ สินค้า เอง)"
            )
