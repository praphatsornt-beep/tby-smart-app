...
- **🔢 คำนวณยอด**: parses LINE-OA-style order text (`calc_logic.parse_calc_order`, e.g. `TF2581-2 RB2306-1 SH-kg12170 COD`), shows customer-facing quote vs. actual cheapest carrier cost (`carriers.get_shipping_options`), can push the quote via LINE, trigger a trial iShip order, and includes a box-packing planner (`carriers.plan_boxes`). A trailing `plan 2500 2500 1000` token (or shorthand `plan 2500*2 1000`) in the same order text triggers `calc_logic.parse_plan_targets()`/`split_bills_by_pv()` — splits the ordered items across multiple sub-bills, each landing as close as possible to its target PV (randomized greedy, 1000 attempts, ported line-for-line from `gas_line_webhook.js`'s `plan` keyword so the LINE OA bot and this tab agree), leftovers shown separately as "ยังไม่เปิดบิล". Computed once per "คำนวณ" click and cached in `st.session_state["_calc_result"]["plan_result"]` — not recomputed every rerun, since the algorithm is randomized.
