...
**Saving customer addresses (`db.upsert_customer_address`)**: always `insert`s a fresh row keyed only by `id` (edit-in-place reuses the existing row's `id`; a new save always gets a new `uuid4()`) — it does **not** dedupe by phone number. Two different customers can legitimately share one recipient phone (e.g. living at the same address), so saving a new address never deletes another customer's saved address just because the phone matches (fixed 2026-09-01 — the old delete-by-phone logic silently wiped a different customer's address on match). `get_address_by_phone()` (used for autofill when typing a phone in ส่งของ/บันทึกขาย) picks the most-recently-created match via `customer_addresses.created_at` (added via `customer_addresses_add_created_at.sql`, ALTER TABLE) when several customers share the phone. `ui_helpers._warn_duplicate_phone()` still shows an informational (non-blocking) note when the phone matches another customer's saved address, in case of a typo — but saving always proceeds as an additional row, never a silent overwrite.
