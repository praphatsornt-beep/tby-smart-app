...
_CUSTOMER_TYPO_RE = re.compile(
    r"([A-Za-z]{1,4})\s*[-.]{0,2}\s*(\d{4})(?:\s*[-.=]{1,2}\s*|\s+)(\d+(?:\.\d+)?)"
)
...
def _normalize_customer_codes(text: str) -> str:
    ...
    'TF2581 2' (เว้นวรรคเฉยๆ ไม่มีสัญลักษณ์เลย ทั้งหน้าเลขและหน้าจำนวน) — normalize เป็น
    ...
    ล้วนๆ ก็ได้ (ไม่บังคับต้องมีสัญลักษณ์)"""
