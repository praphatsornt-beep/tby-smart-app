...
_CUSTOMER_TYPO_RE = re.compile(r"([A-Za-z]{1,4})\s*[-.]{0,2}\s*(\d{4})\s*[-.=]{1,2}\s*(\d+(?:\.\d+)?)")
...
def _normalize_customer_codes(text: str) -> str:
    ...
    normalize เป็น 'TY2010-1'/'TF2581-1'/'TU2315-2'/'TU3601-1' (รูปแบบมาตรฐาน) ก่อนเข้า
    tokenizer ปกติเสมอ เพราะรหัสสินค้าจริงในระบบเป็น LETTERS+4DIGITS ล้วน ไม่มีขีด/จุด
    คั่นเอง (ตรวจสอบแล้วครบ 98/98 ตัวในระบบ 2026-08-28) — จำกัดตัวเลข 4 หลักเป๊ะ กันชนกับ
    รหัสไปรษณีย์ 5 หลักใน SH-kgXXXXX และเบอร์โทรลูกค้า (ไม่มีตัวอักษรนำหน้า) โดยไม่ตั้งใจ"""
