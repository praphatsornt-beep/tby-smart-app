...
_KEX_BULKY = {  # ราคาจริงของ "KEX Bulky" ตัวจริง (เอกสารเงื่อนไขจริงจากหน้า iShip
    # ผู้ใช้ส่งมาให้ 2026-08-14) — คนละตัวกับ _KEX_JUMBO ด้านล่าง (ที่เคยถูกเข้าใจผิดว่า
    # เป็นตัวเดียวกัน) รองรับ 4-20กก. เท่านั้น (ต่ำกว่า/สูงกว่านี้ไม่ใช้บริการนี้)
    1:25,2:30,3:35,4:39,5:45,6:49,7:55,8:59,9:65,10:69,
    11:73,12:80,13:86,14:93,15:99,16:106,17:113,18:119,19:126,20:132,
    # เอกสารมีตารางต่อถึง 30กก. แต่เงื่อนไขระบุชัดว่า "รองรับพัสดุน้ำหนักสูงสุดถึง 20 กก."
    # (เกินนี้คิดค่าบริการเพิ่ม 350 บาท ไม่ใช่ราคาปกติที่ตารางนี้แสดง) — เก็บไว้เผื่ออ้างอิง
    # แต่ max_kg ใน _CARRIER_DEFS ตั้งไว้ที่ 20 ตามเงื่อนไขจริง ไม่ใช่ตามความยาวตาราง
    21:149,22:165,23:182,24:198,25:215,26:231,27:248,28:264,29:281,30:297,
}
...
_KEX_JUMBO = {  # flat rate everywhere, min 30kg use — เดิมชื่อ _KEX_BULKY แต่ตารางนี้เป็น
    ...
    # สร้างตารางนี้ถูก label ว่า "KEX Jumbo" ตั้งแต่ต้น) — เลขทุกตัวยืนยันซ้ำอีกรอบ 2026-08-14
    # ตรงกับเอกสารจริงของ KEX Jumbo เป๊ะ ไม่ต้องแก้
...
_DHL_SIZE_CM = {  # กว้าง+ยาว+สูง รวมสูงสุดต่อน้ำหนัก (ซม.) — ไม่ได้ไล่ทีละ 5cm/kg สม่ำเสมอ
    # เหมือน Flash (โตแบบขั้นบันไดไม่เท่ากันช่วง 1-13kg) แต่ตั้งแต่ 14kg ขึ้นไปไล่ +5cm/kg
    # แบบเดียวกับเจ้าอื่น จนถึง 34kg แล้วกระโดดไปแตะเพดาน 250cm ที่ 35kg (แถวสุดท้าย/
    # น้ำหนักสูงสุดที่รองรับ) — ยืนยันจากตารางจริงของ DHL eCommerce บน iShip (2026-08-05)
    1:70,2:80,3:80,4:80,5:90,6:90,7:100,8:100,9:110,10:110,
    11:110,12:120,13:120,14:125,15:130,16:135,17:140,18:145,19:150,20:155,
    21:160,22:165,23:170,24:175,25:180,26:185,27:190,28:195,29:200,30:205,
    31:210,32:215,33:220,34:225,35:250,
}
...
_CARRIER_DEFS = [
    ...
    # min_kg=4 (เอกสาร: "ใช้ได้สำหรับพัสดุที่มีน้ำหนัก 4 กก. ขึ้นไป"), max_kg=20 (เอกสาร:
    # "รองรับพัสดุน้ำหนักสูงสุดถึง 20 กก." — เกินนี้คิดเพิ่ม 350 บาท ไม่ใช่ราคาปกติ ไม่โมเดล
    # ค่าปรับนี้ ปล่อยให้ exceeds_max กรองออกแทน) sur_fn ใช้ _no_sur เพราะเงื่อนไขบอกว่า
    # "คิดค่าพื้นที่ห่างไกล" จริง แต่ไม่มีรายชื่อ/จำนวนที่ยืนยันแยกสำหรับตัวนี้โดยเฉพาะ —
    # ห้ามใช้ KEX_BULKY_REMOTE (flash_zones.py) ซ้ำ เพราะชุดนั้นน่าจะมาจากเอกสารชุดเดียวกับ
    # ที่เคยเข้าใจผิดว่าเป็น Bulky (จริงคือของ Jumbo) ไม่ใช่ของตัวนี้
    ("kex_bulky",         "KEX Bulky",          _KEX_BULKY,           20,  _no_sur,                 3, 2.675,False, 4,   180, True,  50000, False, 6000),
